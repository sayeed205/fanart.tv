...
describe('MovieModule', () => {
    ...
    describe('get()', () => {
        it('should retrieve movie artwork for valid movie ID', async () => {
            ...
            const artworkTypes = [
                'movieposter', 'moviebackground', 'movielogo',
        ...
        it('should return properly typed response with correct structure', async () => {
            ...
            expect(result.imdb_id === undefined || typeof result.imdb_id === 'string').toBe(true);
            ...
            if (result.movieposter) {
                ...
                if (result.movieposter.length > 0) {
                    ...
                    expect(poster.size === undefined || typeof poster.size === 'string').toBe(true);
                }
            }
            ...
            if (result.moviebackground) {
                ...
                if (result.moviebackground.length > 0) {
                    ...
                    expect(background.size === undefined || typeof background.size === 'string').toBe(true);
                }
            }
        });
        ...
        it('should throw error for invalid movie ID', async () => {
            await expect(movieModule.get(-1)).rejects.toThrow('Movie ID must be a positive integer');
            await expect(movieModule.get(0)).rejects.toThrow('Movie ID must be a positive integer');
            await expect(movieModule.get(1.5 as any)).rejects.toThrow('Movie ID must be a positive integer');
        });

        it('should throw NotFoundError for non-existent movie ID', async () => {
            // Using a very high ID that likely doesn't exist
            const nonExistentId = 99999999;
            await expect(movieModule.get(nonExistentId)).rejects.toThrow(NotFoundError);
        });
    });
    ...
    describe('latest()', () => {
        ...
        it('should throw error for invalid date format', async () => {
            await expect(movieModule.latest('invalid-date')).rejects.toThrow('Date must be in YYYY-MM-DD format');
            await expect(movieModule.latest('2024-1-1')).rejects.toThrow('Date must be in YYYY-MM-DD format');
            await expect(movieModule.latest('24-01-01')).rejects.toThrow('Date must be in YYYY-MM-DD format');
        });
    });
