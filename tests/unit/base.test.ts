...
describe('Fanart Client', () => {
  ...
  describe('constructor', () => {
    ...
    it('should throw error with null API key', () => {
      expect(() => new Fanart(null as any)).toThrow('API key is required and must be a non-empty string');
    });

    it('should throw error with undefined API key', () => {
      expect(() => new Fanart(undefined as any)).toThrow('API key is required and must be a non-empty string');
    });
  ...
  describe('preview functionality integration', () => {
    ...
    it('should handle preview option across all modules', async () => {
      ...
      const artistResult = await fanart.music.artists.get('5b11f4ce-a62d-471e-81fc-a69a8278c7da', { usePreview: true });
      if (artistResult.artistbackground && artistResult.artistbackground.length > 0) {
  ...
  describe('error handling consistency', () => {
    ...
    it('should throw consistent error types across modules', async () => {
      ...
      await expect(fanart.movie.get(99999999)).rejects.toThrow('Resource not found');
      await expect(fanart.music.artists.get('00000000-0000-0000-0000-000000000000')).rejects.toThrow('Resource not found');
      await expect(fanart.tv.get(99999999)).rejects.toThrow('Resource not found');
    });
  ...
  describe('type safety validation', () => {
    ...
    it('should return properly typed responses', async () => {
      ...
      const artistResult = await fanart.music.artists.get('5b11f4ce-a62d-471e-81fc-a69a8278c7da');
