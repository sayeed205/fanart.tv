...
describe("TvModule", () => {
  ...
  describe("get()", () => {
    ...
    it("should throw error for invalid TVDB ID", async () => {
      await expect(tvModule.get(-1)).rejects.toThrow(
        "TVDB ID must be a positive integer",
      );
      await expect(tvModule.get(0)).rejects.toThrow(
        "TVDB ID must be a positive integer",
      );
      await expect(tvModule.get(1.5 as any)).rejects.toThrow(
        "TVDB ID must be a positive integer",
      );
    });
  ...
  describe("latest()", () => {
    ...
    it("should throw error for invalid date format", async () => {
      await expect(tvModule.latest("invalid-date")).rejects.toThrow(
        "Date must be in YYYY-MM-DD format",
      );
      await expect(tvModule.latest("2024-1-1")).rejects.toThrow(
        "Date must be in YYYY-MM-DD format",
      );
      await expect(tvModule.latest("24-01-01")).rejects.toThrow(
        "Date must be in YYYY-MM-DD format",
      );
    });
  });
  ...
  describe("authentication errors", () => {
    it("should throw AuthenticationError with invalid API key", async () => {
      ...
      await expect(invalidModule.get(81189)).rejects.toThrow(
        AuthenticationError,
      );
    });
