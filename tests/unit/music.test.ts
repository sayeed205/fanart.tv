...
describe('MusicModule', () => {
  ...
  describe('ArtistsModule', () => {
    describe('get()', () => {
      it('should retrieve artist artwork for valid MBID', async () => {
        ...
        const artworkTypes = [
          'artistbackground', 'artistthumb', 'musiclogo', 
          'hdmusiclogo', 'musicbanner'
        ];
      ...
      it('should handle preview option correctly', async () => {
        const fullSizeResult = await artistsModule.get(testArtistMbid, { usePreview: false });
        const previewResult = await artistsModule.get(testArtistMbid, { usePreview: true });

        expect(fullSizeResult).toBeDefined();
        expect(previewResult).toBeDefined();

        // Check that preview URLs contain the preview suffix
        if (previewResult.artistbackground && previewResult.artistbackground.length > 0) {
        ...
        if (fullSizeResult.artistbackground && fullSizeResult.artistbackground.length > 0) {
      ...
      it('should throw error for invalid MBID', async () => {
        await expect(artistsModule.get('')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
        await expect(artistsModule.get('   ')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
        await expect(artistsModule.get(null as any)).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      });

      it('should throw NotFoundError for non-existent MBID', async () => {
        const nonExistentMbid = '00000000-0000-0000-0000-000000000000';
        await expect(artistsModule.get(nonExistentMbid)).rejects.toThrow(NotFoundError);
      });
    });
    ...
    describe('latest()', () => {
      it('should retrieve latest artist artwork', async () => {
        try {
          ...
          if (error instanceof FanartApiError && error.message.includes('Invalid JSON')) {
            // Skip this test if the API returns empty response
            console.warn('Latest artists endpoint returned empty response, skipping test');
          } else {
            throw error;
          }
        }
      ...
      it('should retrieve latest artist artwork with date filter', async () => {
        try {
          ...
          if (error instanceof FanartApiError && error.message.includes('Invalid JSON')) {
            // Skip this test if the API returns empty response
            console.warn('Latest artists endpoint with date returned empty response, skipping test');
          } else {
            throw error;
          }
        }
      ...
      it('should throw error for invalid date format', async () => {
        await expect(artistsModule.latest('invalid-date')).rejects.toThrow('Date must be in YYYY-MM-DD format');
        await expect(artistsModule.latest('2024-1-1')).rejects.toThrow('Date must be in YYYY-MM-DD format');
      });
    });
  });
  ...
  describe('album()', () => {
    ...
    it('should handle preview option correctly or handle not found', async () => {
      try {
        const fullSizeResult = await musicModule.album(testAlbumMbid, { usePreview: false });
        const previewResult = await musicModule.album(testAlbumMbid, { usePreview: true });
    ...
    it('should throw error for invalid MBID', async () => {
      await expect(musicModule.album('')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      await expect(musicModule.album('   ')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
    });

    it('should throw NotFoundError for non-existent album MBID', async () => {
      const nonExistentMbid = '00000000-0000-0000-0000-000000000000';
      await expect(musicModule.album(nonExistentMbid)).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('label()', () => {
    ...
    it('should handle preview option correctly', async () => {
      const fullSizeResult = await musicModule.label(testLabelMbid, { usePreview: false });
      const previewResult = await musicModule.label(testLabelMbid, { usePreview: true });
    ...
    it('should throw error for invalid MBID', async () => {
      await expect(musicModule.label('')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      await expect(musicModule.label('   ')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
    });

    it('should throw NotFoundError for non-existent label MBID', async () => {
      const nonExistentMbid = '00000000-0000-0000-0000-000000000000';
      await expect(musicModule.label(nonExistentMbid)).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('authentication errors', () => {
    it('should throw AuthenticationError with invalid API key', async () => {
      ...
      await expect(invalidModule.artists.get(testArtistMbid)).rejects.toThrow(AuthenticationError);
      await expect(invalidModule.album(testAlbumMbid)).rejects.toThrow(AuthenticationError);
      await expect(invalidModule.label(testLabelMbid)).rejects.toThrow(AuthenticationError);
    });
  });
