...
describe('Error Handling', () => {
  ...
  describe('Custom Error Classes', () => {
    ...
    it('should use default messages when none provided', () => {
      ...
      expect(rateLimitError.message).toBe('Rate limit exceeded. Please try again later.');
  ...
  describe('Authentication Error Scenarios', () => {
    it('should throw AuthenticationError with completely invalid API key', async () => {
      ...
      await expect(invalidFanart.movie.get(550)).rejects.toThrow(AuthenticationError);
      await expect(invalidFanart.music.artists.get('5b11f4ce-a62d-471e-81fc-a69a8278c7da')).rejects.toThrow(AuthenticationError);
      await expect(invalidFanart.tv.get(81189)).rejects.toThrow(AuthenticationError);
    });
    ...
    it('should throw AuthenticationError with empty string API key', async () => {
      ...
      expect(() => new Fanart('')).toThrow('API key is required and must be a non-empty string');
    });
  ...
  describe('Input Validation Errors', () => {
    it('should throw validation errors for invalid movie IDs', async () => {
      await expect(fanart.movie.get(-1)).rejects.toThrow('Movie ID must be a positive integer');
      await expect(fanart.movie.get(0)).rejects.toThrow('Movie ID must be a positive integer');
      await expect(fanart.movie.get(1.5 as any)).rejects.toThrow('Movie ID must be a positive integer');
    });

    it('should throw validation errors for invalid TV show IDs', async () => {
      await expect(fanart.tv.get(-1)).rejects.toThrow('TVDB ID must be a positive integer');
      await expect(fanart.tv.get(0)).rejects.toThrow('TVDB ID must be a positive integer');
      await expect(fanart.tv.get(1.5 as any)).rejects.toThrow('TVDB ID must be a positive integer');
    });

    it('should throw validation errors for invalid MBIDs', async () => {
      await expect(fanart.music.artists.get('')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      await expect(fanart.music.artists.get('   ')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      await expect(fanart.music.artists.get(null as any)).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      
      await expect(fanart.music.album('')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
      await expect(fanart.music.label('')).rejects.toThrow('MusicBrainz ID is required and must be a non-empty string');
    });

    it('should throw validation errors for invalid date formats', async () => {
      await expect(fanart.movie.latest('invalid-date')).rejects.toThrow('Date must be in YYYY-MM-DD format');
      await expect(fanart.movie.latest('2024-1-1')).rejects.toThrow('Date must be in YYYY-MM-DD format');
      await expect(fanart.movie.latest('24-01-01')).rejects.toThrow('Date must be in YYYY-MM-DD format');
      
      await expect(fanart.music.artists.latest('invalid-date')).rejects.toThrow('Date must be in YYYY-MM-DD format');
      await expect(fanart.tv.latest('invalid-date')).rejects.toThrow('Date must be in YYYY-MM-DD format');
    });
  });
    ...
    it('should include response data in API errors when available', async () => {
      try {
        ...
        if (error instanceof FanartApiError) {
          ...
          expect(error.response !== undefined || error.response === undefined).toBe(true);
        }
      }
    });
  });
