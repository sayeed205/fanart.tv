...
async function movieExample() {
  ...
  try {
    ...
    for (const { key, name } of artworkTypes) {
      const artwork = movieArt[key as keyof MovieImages] as
        | Array<{ url: string; likes: string }>
        | undefined;
      if (artwork?.length) {
...
async function musicArtistExample() {
  ...
  try {
    ...
    for (const { key, name } of artworkTypes) {
      const artwork = artistArt[key as keyof ArtistImages] as
        | Array<{ url: string; likes: string }>
        | undefined;
      if (artwork?.length) {
        console.log(`${name}: ${artwork.length} images`);
    ...
    console.log(
      `Found ${artistIds.length} artists with recent artwork updates`,
    );
  } catch (error) {
    console.error("❌ Error:", error.message);
  }
...
async function tvShowExample() {
  ...
  try {
    ...
    for (const { key, name } of artworkTypes) {
      const artwork = showArt[key as keyof ShowImages] as
        | Array<{ url: string; likes: string }>
        | undefined;
      if (artwork?.length) {
        console.log(`${name}: ${artwork.length} images`);
...
async function advancedUsageExample() {
  ...
  for (const movieId of movieIds) {
    try {
      ...
      console.log(
        `✅ ${movieArt.name}: ${movieArt.movieposter?.length || 0} posters`,
      );
    } catch (error) {
  ...
  try {
    ...
    console.log(
      `Movies with artwork since ${since}: ${Object.keys(recentMovies).length}`,
    );
  } catch (error) {
