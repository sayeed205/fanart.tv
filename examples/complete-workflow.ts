...
const mediaLibrary: MediaItem[] = [
  ...
  {
    id: "f5093c06-23e3-404f-afe0-f259bbc4b5b6",
    name: "Nevermind",
    type: "album",
  },
  {
    id: "9ba659bd-5ec7-4e3e-8edc-72d08e5e9e7e",
    name: "Parachutes",
    type: "album",
  },
...
class MediaLibraryManager {
  ...
  private async processMovie(item: MediaItem): Promise<MediaItem> {
    ...
    console.log(
      `✅ ${item.name}: ${artwork.posters.length} posters, ${artwork.backgrounds.length} backgrounds`,
    );
  ...
  private async processArtist(item: MediaItem): Promise<MediaItem> {
    ...
    this.stats.successful++;
    console.log(
      `✅ ${item.name}: ${artwork.backgrounds.length} backgrounds, ${artwork.logos.length} logos`,
    );
  ...
  private async processAlbum(item: MediaItem): Promise<MediaItem> {
    ...
    this.stats.successful++;
    console.log(
      `✅ ${item.name}: ${artwork.posters.length} covers, ${artwork.backgrounds.length} CD art`,
    );
  ...
  private async processTvShow(item: MediaItem): Promise<MediaItem> {
    ...
    this.stats.successful++;
    console.log(
      `✅ ${item.name}: ${artwork.posters.length} posters, ${artwork.logos.length} logos`,
    );
  ...
  async demonstrateLatestArtwork() {
    ...
    try {
      ...
      console.log(
        `📽️  Latest movies: ${Object.keys(latestMovies).length} updates`,
      );
      console.log(
        `🎵 Latest artists: ${Object.keys(latestArtists).length} updates`,
      );
      console.log(
        `📺 Latest TV shows: ${Object.keys(latestShows).length} updates`,
      );
  ...
  async demonstrateAdvancedFeatures(results: MediaItem[]) {
    ...
    for (const [type, items] of Object.entries(byType)) {
      ...
      console.log(
        `${type}: ${successful.length}/${items.length} successful, ${totalArtwork} total images`,
      );
    }
